@php use SkillDo\Cms\Support\Metadata; @endphp
<tr class="js_column tr_{{ $item->id}} js_rating_star_table_item rating-star-item {{ ($item->is_read == 0) ? 'is_read' : ''}}"
    data-item="{{ htmlentities(json_encode($item->toArray()))}}">
    <td style="width:600px;" class="js_rating_star_table_review">
        <div class="review-item-info d-flex gap-2">
            <div class="review-item-avatar">
                <div class="avatar"><span>{{ \RatingStar\Supports\RatingStarHelper::getKeyName($item->name)}}</span>
                </div>
            </div>
        ...
        @if(hasItems($files))
            <div class="attach-images">
                @foreach ($files as $path => $file)
                    <div class="images"><a href="{{ Url::base($path)}}"
                                           data-fancybox="group">{!! Template::img(Url::base($path))!!}</a></div>
                @endforeach
            </div>
        @else
            {{ 'không có'  }}
    ...
    <td style="width:290px;" class="text-right js_column_action">
        @if($item->status == 'pending' ||$item->status == 'hidden')
            <button class="btn-green btn btn-green-bg js_rating_star_btn_status p-1 ps-2 pe-2" data-id="{{ $item->id}}">
                Hiển thị
            </button>
        @endif
        <button class="btn-red btn btn-red-bg js_rating_star_btn_reply p-1 ps-2 pe-2" data-id="{{ $item->id}}">Trả lời
        </button>
        <button class="btn-blue btn btn-blue-bg js_rating_star_btn_edit p-1 ps-2 pe-2" data-id="{{ $item->id}}">Chỉnh
            sửa
        </button>
        <button class="btn-red btn js_rating_star_btn_delete p-1 ps-2 pe-2"
                data-id="{{ $item->id}}">{!! Admin::icon('delete')!!}</button>
    </td>
</tr>