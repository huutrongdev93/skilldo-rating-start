@php use RatingStar\Modules\Web\RatingStarProduct;use SkillDo\Cms\Support\Metadata; @endphp
<div class="review-item js_review_item" data-id="{{ $review->id }}" data-name="{{ $review->name }}"
     data-message="{{ $review->message }}">
    <div class="review-comment">
        <div class="rvc_user">
            <div class="rvc_user-avatar">
                <div class="avatar" data-name="{{ $review->name }}">
                    <span>{{ \RatingStar\Supports\RatingStarHelper::getKeyName($review->name) }}</span></div>
            </div>
            <div class="rvc_user-info">
                <div class="rvc_user-name">
                    <span>{{ $review->name }}</span>
                    @if($type == 'products')
                        <div class="rvc_user-seller"><i class="fal fa-check"></i> {{ trans('rating-star::review.seller') }}
                        </div>
                    @endif
                </div>
                <div class="rvc_star">
                    @php RatingStarProduct::template(1,$review->star, config('rating-star::theme.color.star.detail')) @endphp
                </div>
            </div>
        </div>
        <div class="rvc_content">
            <div class="rvc_content_inner">
                <p class="rvc_title">{{ \RatingStar\Supports\RatingStarHelper::starLabel($review->star) }}</p>
                    ...
                    @if(hasItems($files))
                        <div class="attach-images">
                            @foreach ($files as $path => $item)
                                <div class="images">
                                    <a href="{{ Url::base($path) }}"
                                       data-fancybox="group">{!! Template::img(Url::base($path)) !!}</a>
                                </div>
                            @endforeach
                        </div>
                    @endif
                ...
                <div class="rvc_time">
                    <span>{{ \RatingStar\Supports\RatingStarHelper::timeElapsed($review->created) }} trước</span>
                </div>
            </div>
            ...
            @if($reply == true)
                <span class="rvc_reply js_rvc_btn_reply" data-reply-id="{{ $review->id }}">
			        {{ trans('rating-star::review.reply.send') }}
		        </span>
            @endif
            @if(hasItems($review->reply))
                    ...
                    @foreach ($review->reply as $comment)
                        <div class="review-sub-comment">
                            <div class="review-sub-comment__avatar-thumb">
                                <div class="avatar" data-name="{{ $review->name }}">
                                    <span>{{ \RatingStar\Supports\RatingStarHelper::getKeyName($comment->name) }}</span>
                                </div>
                            </div>
                            <div class="review-sub-comment__inner">
                                <div class="review-sub-comment__avatar">
                                    ...
                                    <div class="review-sub-comment__avatar-date">{{ \RatingStar\Supports\RatingStarHelper::timeElapsed($comment->created) }}</time></div>
                                </div>
