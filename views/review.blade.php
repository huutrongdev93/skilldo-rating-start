<div class="review-comments js_review_comments" data-id="{{ $object->id }}" data-type="{{ $type }}">
    ...
    <div class="review-comments_heading">
        <p class="review_heading__title">{{ trans('rating-star::review.title') }}</p>
        <p class="review_heading__close js_review_btn__close"><i class="fal fa-times"></i></p>
    </div>
    <div class="review-comments_sort">
        <ul>
            <li><span>{{ trans('rating-star::review.sort') }}:</span></li>
            <li class="star active" data-sort=""><span>{{ trans('rating-star::review.sort.all') }}</span></li>
            @for($i =1; $i <= 5; $i++)
    ...
    <div class="review-comments_more"><button class="btn btn-theme btn-effect-default js_review_btn__more">{{ trans('rating-star::review.view.all') }}</button></div>
</div>
    ...
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
        <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
            <header class="modal__header">
                <h2 class="modal__title">{{ trans('rating-star::review.reply.title') }}</h2>
                ...
                <form method="post" id="js_review_form__reply" autocomplete="off">
                    <div class="row">
                        <div class="form-group col-md-12">
                            <textarea name="rating_star_message" class="form-control" rows="5" required placeholder="{{ trans('rating-star::template.message.placeholder') }}"></textarea>
                        </div>
                        @if(!Auth::check())
    ...
    $(function(){
		class RatingStartReviewHandle {
			...
			load() {
				...
				let data = {
					'action'        : 'RatingStar\\Ajax\\Web\\RatingStarAjax::reviewLoad',
					'page'          : this.page,
			...
			like(element) {
				...
				let data = {
					'action' : 'RatingStar\\Ajax\\Web\\RatingStarAjax::reviewLike',
					'id' : this.reviewId
				};
			...
			reply(element) {
				...
				data.action = 'RatingStar\\Ajax\\Web\\RatingStarAjax::reviewReply';
