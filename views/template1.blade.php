    ...
    <div class="rating-star-review">
        <div class="rsr__left">
            ...
            @for( $i = 5; $i > 0 ; $i--)
                ...
                @else
                    @php
                        $count_item_star = \RatingStar\Models\RatingStar::where('star', $i)->where('object_type', $type)->where('object_id', $object->id)->count();
        ...
        <div class="rsr__right">
            <div class="rating-reviews__success">
                <div class="text-center">
                    {!! Image::url(Url::base(asset('rating-star::images/success.png')))->html()!!}
                    <p>{{ trans('rating-star::template.success') }}.</p>
                </div>
            </div>
                ...
                <div class="row">
                    @if(!Auth::check())
                        <div class="form-group col-md-6">
                            <input name="rating_star_name" value="{{ $form['name']}}" type="text" class="form-control" placeholder="{{ trans('rating-star::template.name.placeholder') }}" required @if(Auth::check()) {{'readonly'}} @endif>
                        </div>
                        <div class="form-group col-md-6">
                            <input name="rating_star_email" value="{{ $form['email']}}" type="email" class="form-control" placeholder="{{ trans('rating-star::template.email.placeholder') }}" required @if(Auth::check()) {{'readonly'}} @endif>
                        </div>
                    @endif
                    <div class="form-group col-md-12">
                        <textarea name="rating_star_message" class="form-control" rows="5" required placeholder="{{ trans('rating-star::template.message.placeholder', ['name' => $objectName]) }}"></textarea>
                    </div>

                    <div class="form-group col-md-12 review-attach-box">
                        <div class="review-attach-text"><span class="btn-attach js_rating_star_insert_attach">{{ trans('rating-star::template.attach') }}</span></div>
                        <div class="review-attach-list">
                            ...
                            <div class="col-md-12">
                                <p style="text-align: center; font-size: 12px; margin-top: 10px;">{{ trans('rating-star::template.attach.rule') }}</p>
                            </div>
                        </div>
                    </div>
...
<style>
    ...
    .review-attach-box span.btn-attach {
        ...
        display: block;
        background: transparent url('{{ asset('rating-star::images/icon-image.png') }}') no-repeat left center;
...
<script type="text/javascript" defer>
    $(function(){
        ...
        $('#rating-reviews__form').submit(function(){
            ...
            const fileInput = form.querySelector('input.input__upload');

            const fileData = fileInput?.files[0] || null;
            ...
            //formData.append('data', JSON.stringify($(form).serializeJSON()));

            formData.append('action', 'RatingStar\\Ajax\\Web\\RatingStarAjax::reviewSave');

            request.post(ajax, formData, {
                headers: {
                ...
                .then(response => {
                    ...
                    {
                        document.querySelector('.rating-star-form #rating-reviews__form').style.display = 'none';
                        document.querySelector('.rating-star-form .rating-reviews__success').style.display = 'block';
                        form.reset();
                    }
                    else
                    {
                ...
                .catch(error =>
                {
                    SkilldoMessage.error('Đã xảy ra lỗi, vui lòng thử lại.');
                })
                .finally(() =>
                {
                    form.reset();
